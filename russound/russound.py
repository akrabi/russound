...
class Russound:
    ...
    def is_connectable(self):
        """ Check if we can connect to the tcp gateway using the specified host and port. Mainly used to validate the provided parameters """
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            return self.__connect(sock)
    ...
    def set_power(self, controller, zone, power):
        ...
        with self.lock:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                if (self.__connect(sock)) is False:
                    return
                self.__send_data(sock, send_msg)
                _LOGGER.debug("Zone %s - sent message %s", zone, send_msg)
                self.__get_response_message(sock)  # Clear response buffer
    ...
    def set_volume(self, controller, zone, volume):
        ...
        send_msg = self.__create_send_message("F0 @cc 00 7F 00 00 @kk 05 02 02 00 00 F1 21 00 @pr 00 @zz 00 01", controller, zone, volume // 2)
        with self.lock:
            _LOGGER.debug('Zone %s - acquired lock', zone)
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                if (self.__connect(sock)) is False:
    ...
    def set_source(self, controller, zone, source):
        ...
        with self.lock:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                ...
                self.__get_response_message(sock)
    ...
    def get_zone_info(self, controller, zone, return_variable):
        ...
        with self.lock:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                ...
                matching_message = self.__get_response_message(sock, resp_msg_signature)
    ...
    def __send_data(self, sock, data, delay=COMMAND_DELAY):
        ...
        for item in data:
            ...
            try:
                sock.send(data)
            except (ConnectionResetError, ConnectionAbortedError) as msg:
                _LOGGER.error("Error trying to connect to Russound controller. "
    ...
    def __get_response_message(self, sock, resp_msg_signature=None, delay=COMMAND_DELAY):
        ...
        sock.setblocking(0)  # Needed to prevent request for waiting indefinitely
